...
describe('custom head files', () => {
  test('accepts both forms and preserves the order of repeated files', () => {
    const options = parseCliArguments([
      'document-id', 'output',
      '--head-file', 'shared head.html',
      '--head-file=collection.html'
    ]);
    expect(options.headFiles).toEqual(['shared head.html', 'collection.html']);
  });

  test.each([
    ['--head-file'],
    ['--head-file='],
    ['--head-file', '--script', 'nav.js']
  ])('rejects missing head file paths in %j', (...args) => {
    expect(() => parseCliArguments(['document-id', 'output', ...args]))
      .toThrow('--head-file requires a file path');
  });
});
