...
describe('paragraph layout', () => {
  ...
  test('preserves RTL title bottom leading with the standard spacing preset', async () => {
    const registry = new StyleRegistry();
    await renderParagraph(
      {
        paragraphStyle: {
  ...
  test('uses the compact Docs strut for a single-spaced Subtitle', async () => {
    const registry = new StyleRegistry();
    await renderParagraph(
      {
        paragraphStyle: { namedStyleType: 'SUBTITLE', lineSpacing: 100 },
        elements: [{
          textRun: {
            content: 'Subtitle\n',
    ...
    expect(registry.toCSS()).toContain('line-height:0.875;');
  });
