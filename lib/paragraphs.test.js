...
describe('paragraph layout', () => {
  ...
  test('uses font metrics for the compact Docs title preset', async () => {
    const registry = new StyleRegistry();
    await renderParagraph(
      {
        paragraphStyle: { namedStyleType: 'TITLE' },
    ...
    expect(registry.toCSS()).toContain('margin-bottom:8px;');
    expect(registry.toCSS()).not.toContain('padding-bottom:');
  });
